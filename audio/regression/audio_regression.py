...
X = df_allfeatures['all_features_concat'].values.tolist()
y = df.PHQ8_Score
...
reg_selector = SelectKBest(score_func=f_regression, k=160)
...
# save data sets
# train_x = open('X_train_audio_r.pickle', 'wb')
# pickle.dump(X_train, train_x)
# train_x.close()
#
# train_y = open('y_train_audio_r.pickle', 'wb')
# pickle.dump(y_train, train_y)
# train_y.close()
#
# test_x = open('X_test_audio_r.pickle', 'wb')
# pickle.dump(X_test, test_x)
# test_x.close()
#
# test_y = open('y_test_audio_r.pickle', 'wb')
...
def k_cross_validation(model, grid, X=X_train, y=y_train):
    ...
    # save best model
    # file = open(str(model) + '_audio_r', 'wb')
...
def random_forest():
    ...
    # try:
    #     file = open(str(model) + '_audio_r', 'rb')
...
# def mlp():
#     grid = {'hidden_layer_sizes': [(100,), (200,), (300,)], 'activation': ['tanh', 'relu'],
